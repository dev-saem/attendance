  ...
  for (let i = 0; i < stu.length; i++) {
    ...
    for (let j = 0; j < stu.length; j++) {
      if (arr[i].id == num[j]) {
        ...
        arr[i].innerText = stu[j];

        if (params[j].runOut == 0){
          if (00 < minuteTime[j] && minuteTime[j] < 11) {
            arr[i].style.backgroundColor = "blue";
            params[j].attend = 2;
            // 00분 초과 11분 미만일 때, 둘 다 포함일때
  
          } else if (49 < minuteTime[j] || minuteTime[j] == 00) {
            arr[i].style.backgroundColor = "green";
            params[j].attend = 1;
            // 49분 초과이거나, 00분일 때
  
          } else if (10 < minuteTime[j] || minuteTime[j] < 50) {
